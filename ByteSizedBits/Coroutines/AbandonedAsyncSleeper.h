...
struct AbandonedSleeperTask
{
    ...
    AbandonedSleeperTask(handle_type handleType) : coro(handleType) {}
    ~AbandonedSleeperTask() { if (coro) { coro.destroy(); } }
    void resume() { if (coro && !coro.done()) { coro.resume(); } }
    bool done() const { return !coro || coro.done(); }

    struct promise_type
    {
        AbandonedSleeperTask get_return_object() { return AbandonedSleeperTask{ handle_type::from_promise(*this) }; }
