...
struct AsyncSleep {
    ...
    void await_suspend(std::coroutine_handle<> h) {
        // Spawn a thread to do the waiting, so the main thread can return
        std::thread([h, d = duration_s]() {
            std::this_thread::sleep_for(std::chrono::seconds(d));
            h.resume(); // Resume strictly AFTER the sleep finishes
            }).detach();
    }
...
FireAndForget AsyncTimer() {
    ...
    co_await AsyncSleep{ 4 };

    // This runs 4 seconds later
    cout << "Time's up\n";
